...
const generateChart = data => {
    ...
    const svg = d3.select('#category-bubble-chart')
        .style('width', width)
        .style('height', height);
    ...
    const circle = node.append('circle')
        ...
        .on('click', (e, d) => {
            window.location.href = `/cat_details?category=${encodeURIComponent(d.data.name)}`;
        });
...
(async () => {
    // get data
    const data = await d3.json(categorydataPath);
